...
async function getRecipes() {
  ...
  let reci = localStorage.getItem("recipes");
  if (reci) {
    return JSON.parse(reci);
  }
  /**************************/
  ...
  // we're returning
  /**************************/
  async function callback_func(resolve, reject) {
  ...
  //            "await" again
        let url_json = await url.json();
  // A8. TODO - Add the new recipe to the recipes array
        fetch_reci.push(url_json);
  ...
  //            resolve() method.
        if (i == (RECIPE_URLS.length - 1)) {
          saveRecipesToStorage(fetch_reci);
          resolve(fetch_reci);
        }
      } catch (error) {
